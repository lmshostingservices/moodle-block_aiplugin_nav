...
class block_aiplugin_nav extends block_base {
    ...
    public function get_content() {
        ...
        /**
     * Legacy dropdown interface, retained for rollback. Replaced by the ainav2 UI in v2.5.0.
     *
     * @return stdClass The block content.
     */
    protected function get_content_legacy() {
    ...
    public function get_required_javascript() {
        ...
        $PAGE->requires->js_amd_inline("
            require(['jquery', 'core/ajax', 'core/notification'], function ($, Ajax, Notification) {
                ...
                // expectedsha256 is typed PARAM_ALPHANUM by the web service, so a value
                // carrying a sha256- prefix, base64 padding or a dash is rejected before
                // the call runs and the update dies with an invalid parameter error.
                // The hash is optional, so drop anything that is not a clean hex digest.
                function ainavSafeSha(v) {
                    var t = String(v || '');
                    return /^[a-fA-F0-9]{64}$/.test(t) ? t : '';
                }
                ...
                function doAutoInstall(btn, component, downloadUrl, pluginName, expectedSha256) {
                    ...
                    Ajax.call([{
                        ...
                        args: {
                            ...
                            expectedsha256: ainavSafeSha(expectedSha256)
                        },
                        done: function (response) {
                            if (response.success) {
                    ...
                    Ajax.call([{
                        methodname: 'block_aiplugin_nav_auto_update_plugin',
                            args: {
                                downloadurl: downloadUrl,
                                component: component,
                                expectedsha256: ainavSafeSha(latestVersions && latestVersions[component] && latestVersions[component].sha256)
                            },
                            done: function (response) {
                    ...
                    var calls = pluginsNeedingUpdate.map(function (plugin) {
                        return {
                            ...
                            args: {
                                ...
                                component: plugin.component,
                                expectedsha256: ainavSafeSha(plugin.sha256)
                            }
                        };
                    });
